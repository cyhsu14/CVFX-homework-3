...
|結果圖|描述|
|:---:|---|
|<img src="./img/giantTree.png">|嘗試要長出一柱擎天的樹<br>失敗，只有地面上多出了一棵沒那麼高的樹，<br>還有被畫到的建築物變高了。|
|<img src="./img/groundTree.png">|嘗試在地面上長出一排矮木<br>成功，雖然長出來的樹只有少少幾株，<br>但在這個很明顯是水泥地的環境的這張圖中，<br>以我的角度看來有太多樹也是不太合理的。|
|<img src="./img/skyTree.png">|嘗試天外飛來一樹<br>失敗，而的確常識上來說天空並不會飄出一棵樹。|
|<img src="./img/doorTree.png">|嘗試在正門口長一棵樹<br>失敗，但是在門口旁長出了樹。|
...
|實驗圖討論|1. 幾乎觀察不到樹的痕跡，推測是因為這個GAN學到了教堂附近樹並不是必要，因此，不會想要補樹。<br>2. 屋簷有不合常理的突起，推測是因為model只有學到屋簷的材質跟屋頂的形狀，屋簷該是平滑的這件事並沒有學到。考量跟屋簷有關的feature map中並沒有indicate說這個地方會有屋簷，是model學到這裡該補屋簷所以才補上來的，generate出這種影像是挺合理的。|1. 圖中的樹並沒有完全被抹除，而天空中髒髒的痕跡是三者中最多。推測是local discriminator 128x128的mask看到的區域中額外的訊息太多反而導致雜訊產生。<br>2. 有嘗試填補屋頂與房屋外牆，但是不自然。推測是因為這個model並不是學到這些空白區域（如這個例子是房屋的外牆）應該長什麼樣子，而是生成與旁邊的pixel相似的質地，所以它產生一些類似窗框的形狀，但由於不知道真正自然的影像應該是如何，所以才會變成這種重疊的不自然痕跡。||
